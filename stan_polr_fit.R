function (x, y, wt = NULL, offset = NULL, method = c("logistic",
    ...
    do_residuals = algorithm == "sampling")
{
    ...
    all_methods <- c("logistic", "probit", "loglog", "cloglog",
        "cauchit")
    ...
    if (!has_weights)
        weights <- double(0)
    has_offset <- isTRUE(length(offset) > 0 && !all(offset ==
        0))
    if (!has_offset)
        offset <- double(0)
    if (length(prior)) {
        regularization <- make_eta(prior$location, prior$what,
            K = ncol(x))
        prior_dist <- 1L
    }
    else {
    ...
    if (!length(prior_counts)) {
        ...
        prior_counts <- maybe_broadcast(prior_counts$concentration,
            J)
    }
    if (is.null(shape)) {
        shape <- 0L
    ...
    standata <- nlist(J, N, K, X, xbar, y, prior_PD, link, has_weights,
        weights, has_offset, offset, prior_dist, regularization,
        prior_counts, is_skewed, shape, rate, has_intercept = 0L,
        prior_dist_for_intercept = 0L, prior_dist_for_aux = 0L,
        dense_X = TRUE, nnz_X = 0L, w_X = double(0), v_X = integer(0),
        u_X = integer(0), prior_dist_for_smooth = 0L, K_smooth = 0L,
        S = matrix(NA_real_, N, 0L), smooth_map = integer(0),
    ...
    if (algorithm == "sampling") {
        sampling_args <- set_sampling_args(object = stanfit,
            prior = prior, user_dots = list(...), user_adapt_delta = adapt_delta,
    ...
    thetas <- extract(stanfit, pars = "beta", inc_warmup = TRUE,
        permuted = FALSE)
    betas <- apply(thetas, 1:2, FUN = function(theta) R_inv %*%
        theta)
    if (K == 1)
        for (chain in 1:tail(dim(betas), 1)) {
            stanfit@sim$samples[[chain]][[(J == 2) + 1L]] <- betas[,
                chain]
        }
    else for (chain in 1:tail(dim(betas), 1)) for (param in 1:nrow(betas)) {
        stanfit@sim$samples[[chain]][[(J == 2) + param]] <- betas[param,
            , chain]
    }
    if (J > 2) {
        new_names <- c(colnames(x), paste(head(y_lev, -1), tail(y_lev,
            -1), sep = "|"), paste("mean_PPD", y_lev, sep = ":"),
            "log-posterior")
    }
    else {
        new_names <- c("(Intercept)", colnames(x), if (is_skewed) "alpha",
            "mean_PPD", "log-posterior")
    }
    stanfit@sim$fnames_oi <- new_names
    prior_info <- summarize_polr_prior(prior, prior_counts, shape,
        rate)
    structure(stanfit, prior.info = prior_info)
}
<bytecode: 0x5574a6ab9570>
