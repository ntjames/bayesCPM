...
#!! find better inits?
fit_il_6 <- bayes_cpm(hiv_dat_il_6, seed=77845, 
                 iter=4000, warmup=2000, chains=2, init="0", 
                 control = list(adapt_delta = 0.95))

if(0){
fit_il_6_nocnt <- bayes_cpm(hiv_dat_il_6_nocnt, seed=77845, 
                 iter=4000, warmup=2000, chains=2, init="0", 
                 control = list(adapt_delta = 0.95))
}

qplot(summary(fit_il_6)$summary[,"Rhat"])
...
summary(fit_il_6, pars=main_pars)$summary %>% round(3) %>% set_rownames(main_pars_labs)

rstan::traceplot(fit_il_6, pars=main_pars) 
...
il_6_posta <- as.matrix(fit_il_6, pars=main_pars[c(1,2,6)])  %>%
mcmc_intervals() + scale_y_discrete(labels =main_pars_labs2[c(1,2,6)] )

il_6_postb <- as.matrix(fit_il_6, pars=main_pars[c(3,4,5,7)])  %>%
...
stan_plot(fit_il_6, pars=main_pars) + 
  scale_y_discrete(breaks=paste0("b[",1:7,"]"),
        labels=letters[1:7])

# cutpoints
il_6_ct_plt<-plot(fit_il_6,pars=c(paste0("cutpoints[",1:173,"]")),
                  ci_level = 0.5)
...
if(0){
traceplot(fit_il_6, pars=c(paste0("cutpoints[",1:20,"]")))
traceplot(fit_il_6, pars=c(paste0("cutpoints[",21:40,"]")))
traceplot(fit_il_6, pars=c(paste0("cutpoints[",41:60,"]")))
traceplot(fit_il_6, pars=c(paste0("cutpoints[",61:80,"]")))
traceplot(fit_il_6, pars=c(paste0("cutpoints[",81:100,"]")))
traceplot(fit_il_6, pars=c(paste0("cutpoints[",101:120,"]")))
traceplot(fit_il_6, pars=c(paste0("cutpoints[",121:140,"]")))
traceplot(fit_il_6, pars=c(paste0("cutpoints[",141:160,"]")))
traceplot(fit_il_6, pars=c(paste0("cutpoints[",161:173,"]")))
}

# cell probs
if(0){
plot(fit_il_6,pars=c(paste0("pi[",1:20,"]")))
plot(fit_il_6,pars=c(paste0("pi[",21:40,"]")))
plot(fit_il_6,pars=c(paste0("pi[",41:60,"]")))
plot(fit_il_6,pars=c(paste0("pi[",61:80,"]")))
plot(fit_il_6,pars=c(paste0("pi[",81:100,"]")))
plot(fit_il_6,pars=c(paste0("pi[",101:120,"]")))
plot(fit_il_6,pars=c(paste0("pi[",121:140,"]")))
plot(fit_il_6,pars=c(paste0("pi[",141:160,"]")))
plot(fit_il_6,pars=c(paste0("pi[",161:174,"]")))
}
...
```{r il_6_trans}
cp_il_6 <- summary(fit_il_6, pars=c(paste0("cutpoints[",1:173,"]")))$summary[,"50%"]

cp_il_6_nocnt <- summary(fit_il_6_nocnt, pars=c(paste0("cutpoints[",1:173,"]")))$summary[,"50%"]

il_6_trans_dat <- cbind(hiv_dat_il_6$truey0[2:174], cp_il_6) %>% as.data.frame() %>% rename(`IL-6`=V1)

il_6_trans_dat_nocnt <- cbind(hiv_dat_il_6$truey0[2:174], cp_il_6_nocnt) %>% as.data.frame() %>% rename(`IL-6`=V1)
...
```{r il_6_bmi_dat}
mn_summ_il_6 <- getMean(fit_il_6, hiv_dat_il_6, newdat1)
