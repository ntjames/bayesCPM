...
getCDF <- function(fit, newdata, summ=TRUE, ...){
  ...
  truey <- c(-Inf,fit$standata$truey0)
  
  # format newdata, betas, and intercepts
  #!npreds <- fit@par_dims$b
  cv_nms <- paste0("b[",1:fit$stanfit@par_dims$b,"]")
  cut_nms <- paste0("cutpoints[",1:(fit$standata$ncat-1),"]")
  
  ndr <- newdata %>% dplyr::mutate(ndrow=1:n())
  nd <- ndr %>% dplyr::select(-ndrow) %>% as.matrix()
  
  beta <- posts %>% dplyr::select(all_of(cv_nms)) %>% as.matrix()
  int <- posts %>% dplyr::select(all_of(cut_nms)) %>% as.matrix()
  ...
  if (summ){
    cdf_summ <- cdf_vals %>%
      ...
      dplyr::summarize(mn_cdf=mean(cdf),
                       ...
                       cdf_q97.5=quantile(cdf,probs=0.975), .groups="drop") %>%
      ungroup() %>% # redundant w/ .groups="drop"?
      dplyr::mutate(yval=rep(truey,nrow(nd))) %>%
