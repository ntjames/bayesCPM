...
# sim_c0_n - logistic link, log trans, conc=1/ncats
# sim_c1_n - logistic link, log trans, conc=1/(0.8 + 0.35*max(ncats, 3))
# sim_c2_n - logistic link, log trans, conc=1/(2+(ncats/3))
# sim_c3_n - logistic link, log trans, conc=1/2
...
# sim_beta_summ(sim_c0_n400[['full']]); sim_beta_summ(sim_c0_n400[['cens']])
...
# sim_gamma_summ(sim_c0_n400[['full']]); sim_gamma_summ(sim_c0_n400[['cens']])
# sim_gamma_summ(sim_c0_n25[['full']]); sim_gamma_summ(sim_c0_n25[['cens']])
...
if(0){
  ...
  segments(x0=exp(yvals), y0=rep(0,5),
           y1=pllogis(exp(yvals),shape = 3, scale = exp(1*1-0.5*1)), lty=2)
  segments(x0=rep(-3,5), x1=exp(yvals),
           y0=pllogis(exp(yvals),shape = 3, scale = exp(1*1-0.5*1)), lty=2)
  
  # at z1=0, z2=0
  pllogis(exp(yvals),shape = 3, scale = exp(0))
  curve(pllogis(x,shape = 3, scale = exp(0)),0,6,xlab='y')
  segments(x0=exp(yvals), y0=rep(0,5),
           y1=pllogis(exp(yvals),shape = 3, scale = exp(0)), lty=2)
  segments(x0=rep(-3,5), x1=exp(yvals),
           y0=pllogis(exp(yvals),shape = 3, scale = exp(0)), lty=2)
  
  # on log scale
  plogis(yvals,1*1-0.5*1,1/3)
  curve(plogis(x,1*1-0.5*1,1/3),-3,3, xlab='log(y)')
  segments(x0=yvals, y0=rep(0,5),
           y1=plogis(yvals,1*1-0.5*1,1/3), lty=2)
  segments(x0=rep(-3,5), x1=yvals,
           y0=plogis(yvals,1*1-0.5*1,1/3), lty=2)
  
  # at z1=0, z2=0
  plogis(yvals,0,1/3)
  curve(plogis(x,0,1/3),-3,3, xlab='log(y)')
  segments(x0=yvals, y0=rep(0,5),
           y1=plogis(yvals,0,1/3), lty=2)
  segments(x0=rep(-3,5), x1=yvals,
           y0=plogis(yvals,0,1/3), lty=2)
...
# sim_cdf_summ(sim_c0_n400[['full']]); sim_cdf_summ(sim_c0_n400[['cens']])
...
# sim_mn_summ(sim_c0_n25[['full']]); sim_mn_summ(sim_c0_n25[['cens']])
...
ggsave(file.path(figdir,"sim_c_pars_full.png"),width=pltw,height=plth)
...
ggsave(file.path(figdir,"sim_c_pars_cens.png"),width=pltw,height=plth)
...
ggsave(file.path(figdir,"sim_c_pars.png"),width=pltw,height=plth)
...
ggsave(file.path(figdir,"sim_c_cdf_full.png"),width=pltw,height=plth)
...
ggsave(file.path(figdir,"sim_c_cdf_cens.png"),width=pltw,height=plth)
...
ggsave(file.path(figdir,"sim_c_mn_full.png"),width=pltw,height=plth)
...
ggsave(file.path(figdir,"sim_c_mn_cens.png"),width=pltw,height=plth)
...
ggsave(file.path(figdir,"sim_c_med_full.png"),width=pltw,height=plth)
...
ggsave(file.path(figdir,"sim_c_med_cens.png"),width=pltw,height=plth)
...
ggsave(file.path(figdir,"sim_c_q20_full.png"),width=pltw,height=plth)
...
ggsave(file.path(figdir,"sim_c_q20_cens.png"),width=pltw,height=plth)
...
ggsave(file.path(figdir,"sim_c_q80_full.png"),width=pltw,height=plth)
...
ggsave(file.path(figdir,"sim_c_q80_cens.png"),width=pltw,height=plth)
...
ggsave(file.path(figdir,"sim_c_q80.png"),width=pltw,height=plth)
