...
[ 2) mixture link - probably not most important to include sinc there is evidence from Qi & Yuqi's papers and prev poster that models are reasonably robust to link misspecification]

2) nonlinear model

3) Extend to longitudinal nonlinear CPMs - nonlinear mixed effects model for Bayesian PK/PD

TO DO
- Clean up section describing basic Bayesian ordinal model 

- Write section on extension to continuous, nail down 'proof' related to priors for intercepts/heuristic
-- verify that NPMLE likelihood and my likelihood are the same

- clean up functions to get Mean, Quantile and Exceedance from output
--use roxygen function notation similar to 'Copula modeling with R'

- implement BNP model for comparison (JAGS?? dirichletprocess package, other?)
...
-- switch CPM models to JAGS??

- write code for basic simulations
-- see Yuqi and Qi papers for scenarios, others
-- make sure model is working well in basic scenario before extending!!

- write code for mixed effects model
-- see Sorensen et al 2016 paper and STAN for linear mixed models at https://people.bath.ac.uk/jjf23/stan/

- write code for nonlinear model
...
- write nonlinear mixed effects model
...
[Introduction] - 
Models for ordinal outcomes (ref) and extensions of these models using the Bayesian paradigm have been discussed extensively in the literature (ref).

mini lit review for ordinal models and summarizing results of previous continuous CPM papers

what did we do? - Bayes CPM
why did we do it? - flexibility for outcome mods, bayesian inf
...
The Bayesian cumulative ordinal regression model was described by Albert and Chib [@albert_bayesian_1993; @albert_bayesian_1997] and Johnson and Albert [@johnson_ordinal_1999]. Let $y_1, \ldots, y_n$ be observed outcomes $y_i$ which each fall into one of $J$ ordered categories $1,\ldots, J$ and with $p$ associated covariates $x_i=(x_{i1},\ldots,x_{ip})$. Suppose $Z_i=x_i'\beta  + \epsilon_i$ are continuous latent variables with distribution $\epsilon_i \sim F$ underlying each $y_i$ so the probability of observation $i$ falling into category $j$ given the covariates $x_i$ is $p_{ij}=Pr(\gamma_{j-1} < Z_i < \gamma_j)=F(\gamma_{j} - x_i'\beta) - F(\gamma_{j-1} - x_i'\beta)$ and the cumulative probability of falling into category $j$ or a lower category is $\eta_{ij}=\sum_{k=1}^{j}p_{ik}$. The cumulative probability model is:
\begin{gather}
P(Y \le j|X,\beta,\gamma)= \eta_{ij} = F(\gamma_{j}-\boldsymbol{x_i'\beta}),\, i=1,\ldots,n,\, j=1,\ldots,J-1 
\end{gather}
where $F(\cdot)$ is a specified cumulative distribution function, $\boldsymbol{x_i}$ is a $1 \times p$ covariate vector, $\gamma_j$ are ordered cutpoints which categorize the outcome ($\gamma_0=-\infty$ and $\gamma_n=\infty$), and $\boldsymbol{\beta}$ is a $p \times 1$ vector of coefficients. 

As noted by Johnson & Albert [@johnson_ordinal_1999], the model with $J$ categories and $J-1$ cutpoints is nonidentifiable and there are two approaches to resolve this issue. The first approach fixes the value of one cutoff or equivalently fixes the intercept in the regression function $Z=\boldsymbol{x'\beta} + \epsilon$ to be e.g., 0. Applying noninformative priors for the remaining cutoffs and $\beta$s along with the constraint $\gamma_1 \le \cdots \le \gamma_{J-1}$ results in maximum a posteriori (MAP) estimates which are equivalent to MLE estimates if there are moderate counts observed in each category. 

A second approach defines a probability distribution on the vector of cutpoints. Rather than specifying a prior directly on the $\gamma_i$, it is more intuitive to specify priors on the probability of falling into category $j$ and then use the specified distribution function to transform to the correct scale. If the observed $\boldsymbol{y}=(y_1, \ldots, y_n)$ are assumed independent given $\boldsymbol{p_i}=(p_{i1},\ldots,p_{iJ})$ the likelihood of $\boldsymbol{y}$ is proportional to a multinomial density $p(\boldsymbol{y}|\boldsymbol{p_i}) \propto \prod_{i=1}^{n} p_{ij}$. Using the prior $p(\boldsymbol{\gamma},\boldsymbol{\beta})$ and substituting for $p_{ij}$, the joint posterior distribution for ($\boldsymbol{\gamma},\boldsymbol{\beta})$ is: 
\begin{gather}
p(\boldsymbol{\gamma},\boldsymbol{\beta}|\boldsymbol{x},\boldsymbol{y}) \propto
p(\boldsymbol{\gamma},\boldsymbol{\beta})\prod_{i=1}^{n} p_{ij}=
p(\boldsymbol{\gamma},\boldsymbol{\beta})\prod_{i=1}^{n}[F(\gamma_j-\boldsymbol{x_i}'\beta )-F(\gamma_{j-1}-\boldsymbol{x_i}'\beta)]
\end{gather} 
Because $\boldsymbol{y}$ is multinomial a natural choice for the prior distribution is a conjugate Dirichlet prior $p(\boldsymbol{p_i})\propto \prod_{i=1}^{n}p_i^{\alpha_i-1}$ where $\alpha_i>0$ are $n$ concentration parameters. Using this prior, the concentration parameters can be interpreted as prior 'observations' in the each of the $J$ categories and the posterior is also Dirichlet. For example, using a symmetric Dirichlet distribution with $\alpha_1=\alpha_2=\cdots=\alpha_n=1$ implies a posterior distribution of $p(\boldsymbol{p_i}|y) \propto Dirichlet(1+n_1,1+n_2,\ldots,1+n_J)$ where $n_j$ is the number of observations in each category.

<!--
$\alpha=1$ equally favor all vectprs $\pi$ such that $\sum \pi_j =1$
$\alpha < 0$ favors equally distributed counts
$\alpha > 1$ favors sparsity in most cell with probs grouped in small number of cells
...
Rather than specifying concentration, estimate $\alpha$ using hierarchical model with $\alpha \sim Gamma(a, b)$.  <!--(plot draws from this?)--> Still might have issues since implies all $\alpha_k$ are centered around same location, solve this problem by using dirichlet process mixture 
...
<!---
Remove mixture link section for now unless there is evidence this extension is needed to fit model well. Evidence from previous papers and CPM poster show relatively good robustness to link misspecification
* Mixture link
...
Use Glycohemoglobin in diabetes ex. from RMS with candidate models
