...
The model in (\ref{eq:post}) is implemented using the `R` interface to Stan [@stan_development_team_rstan:_2018] which performs MCMC sampling using Hamiltonian Monte Carlo. [Add a few sentences about HMC??] number iterations, number of chains, burn-in, adapt delta & other settings
...
To evaluate the <!--long-run frequency--> properties of the Bayesian CPM we generate data from several simulation models. Both begin with continuous data from:
\begin{eqnarray*}
...
For each setting a second set of simulations is used to evaluate a mixed discrete/continuous outcome such as a lower limit of detection. For the first two scenarios values of $Y<1$ to were set to 1 and for the third scenario values of $Y<0$ were set to 0. Under each of the six scenarios, $1000$ datasets were generated for sample sizes $n=25,50,100,200$ and $400$.
