...
Let $\pi_{.j} \equiv Pr(r(y)=j|x=0)$ be the probability of being in category $j$ when all the covariates are 0 and $\boldsymbol{\pi_{.}}=(\pi_{.1},\ldots,\pi_{.J})$. From equation \ref{eq:cellprobs} it follows that
\begin{gather}
\pi_{.j}=G(\gamma_j-0)-G(\gamma_{j-1}-0)=G(\gamma_j)-G(\gamma_{j-1})
\end{gather}
These equations define a transformation $h(\boldsymbol{\gamma})=\boldsymbol{\pi_{.}}$ between the cutpoints and probabilities of category membership when all the covariates are 0 (or at their mean values if covariates have been centered). Conversely,
\begin{gather}
...
Because $\boldsymbol{y}$ has a multinomial distribution a conjugate Dirichlet distribution is a natural choice of prior for $\boldsymbol{\pi_{.}}$. Setting $p(\boldsymbol{\pi_{.}}) \propto \prod_{j=1}^{J}\pi_{.j}^{\alpha_j-1}$ the posterior distribution is
\begin{align}
...
&\propto p(\boldsymbol{\pi_{\cdot}})|\mathcal{J}|p(\boldsymbol{\beta}) p(\boldsymbol{y}|\boldsymbol{x},\boldsymbol{\gamma},\boldsymbol{\beta})
\end{align}
...
\begin{align}
\mathcal{J}_{j,1}&=\frac{\partial \pi_{.j}}{\partial \Omega}=1\\
\mathcal{J}_{j+1,j+1}&=\frac{\partial \pi_{.j+1}}{\partial \gamma_j}=\frac{\partial}{\partial \gamma_j}\left[G(\gamma_{j+1})-G(\gamma_{j})\right]=-g(\gamma_j)\\
...
<!--!!!! NB: $G^{-1}()$ is cdf so $g()$ is pdf, need to correct notation throughout so that $G^{-1}()$ is inverse cdf/quantile and $G()$ is cdf
-->
...
The Dirichlet prior on $\boldsymbol{\pi_{.}}$ along with the transformation $h(\cdot)$ induces a prior for the $\boldsymbol{\gamma}$. The choice of $\alpha_j$ controls how concentrated the induced prior is around the MLEs for the cutpoints when $X=0$.

Figure \@ref(fig:probit-induced) shows induced priors

```{r probit-induced, fig.cap="Induced priors", fig.align='center', out.width='90%'}
knitr::include_graphics(file.path(wd,"fig","probit_induced.png"))
```

Assuming a symmetric Dirichlet distribution $\alpha_1 = \alpha_2 = \cdots = \alpha_n$, we explore several possible choices for the $\alpha_j$ parameters

show draws of $\boldsymbol{\gamma}$ induced priors for each of the possible prior choices for $\alpha_j$

[possible choices Uniform 1, Jeffrey's 1/2, Objective?? $\alpha_j =\frac{1}{J}$ for all $j$, 1/(0.8 + 0.35 * max(n, 3))]. 



In multi-parameter models, the optimal choice of reference prior depends on the parameter or statistic of interest (e.g. $\beta$, conditional CDF, conditional mean)  so we seek a value of $\alpha$ that will provide a overall default for a variety of settings and quantities of interest ... following the approach of Berger et al. [@berger_overall_2015].  
...
Using MCMC samples from the posterior distribution ($\tilde{\gamma}$, $\tilde{\beta}$) it is straightforward to calculate the posterior conditional CDF, mean, quantiles or other functionals as shown in the table.
...
To evaluate the long-run frequency properties of the Bayesian CPM we generate data from two simulation models.

The first model for a continuous outcome is:
...
The second simulation model uses identical datasts, but sets values of $Y<0$ to 0 to produce a mixed discrete/continuous outcome. $1000$ datasets were simulated for sample sizes $n=25,50,100,200$ and $400$

A Bayesian CPM using three $\alpha$ concentration hyperparameters for $p(\boldsymbol{\pi_{.}})$ with the properly specified probit link was fit to each simulated dataset across the two outcome models. <!-- For comparison, the NPMLE CPM model [and the BNP model for residuals] were also fit to each dataset. --> The average bias of the posterior medians and standard errors of $\beta_1$ and $\beta_2$ were compared for the three models. In addition, five posterior intercept parameter estimates were compared for the Bayesian and frequentist CPM models.

add sims for conditional CDF, mean, and quantile
...
```{r simplt1, fig.cap="Bias in uncensored simulations", fig.align='center', out.width='90%'}
knitr::include_graphics(file.path(wd,"fig","sim_param_full2.png"))
```

```{r simplt2, fig.cap="Bias in censored simulations", fig.align='center', out.width='90%'}
knitr::include_graphics(file.path(wd,"fig","sim_param_cens2.png"))
```

Describe results from graphs, compare and contrast the prior models
...
- bias for small $n$
- Another potential limitation of the formulation above is that the number of distinct outcome values must be known *a priori*, that is we condition on $J$ categories. Some discussion of extensions using  e.g. Dirichlet process mixture or Polya trees (ref semiparameteric surv. analysis papers?)
- speed?
