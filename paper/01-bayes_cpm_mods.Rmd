...
-> Use model with dirichlet and concentration alpha=1/n
also try alpha=0, alpha=1/2 (is this multinomial Jeffrey's prior or could it be that 1/n is multinomial Jeffreys prior?), alpha=1
describe how to do informative prior based on conditional mean arg?

1b) make sure hardcoded stan model matches stan_polr when alpha=1, 0.5, 0.2, 2, 5, 1/ncat (other vals?)
...
2a) mixture link model (maybe, depends on whether it's truly needed or improves model)
--finite mixture and BNP mixture over, e.g. normals

2b) Extend to nonlinear model

2c) Extend to mixed effects CPMs - nonlinear mixed effects model for Bayesian PK/PD
...
- consider alternate parameterization delta_1=log(gamma_1); delta_j=log(gamma_j-gamma_{j-1}) from Albert & Chib 1997
...
-- extend simulation code to compare Bayes CPM to BNP
...
what did we do? - Bayes CPM for continuous, compare to BNP, extend to nonlinear & mixed effects  
