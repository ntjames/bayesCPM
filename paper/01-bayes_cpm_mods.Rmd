...
\end{gather}
The likelihood for an independent and identically distributed sample of observations $\boldsymbol{y}=(y_1,\ldots,y_n)$ is
\begin{gather}
...
\end{gather}
so $G^{-1}\left(\sum_{k=1}^{j}\pi_{.k}\right)=\gamma_j$ defines the inverse transformation $h^{-1}(\boldsymbol{\pi_{.}})=\boldsymbol{\gamma}$.
...
Using the $s=1,\ldots,S$ MCMC draws from the posterior distribution, ($\tilde{\boldsymbol{\gamma}}^{(s)}$, $\tilde{\boldsymbol{\beta}}^{(s)}$), it is straightforward to calculate the distribution of the posterior conditional CDF, mean, quantiles or other functions of the parameters. For example, the distribution of the posterior conditional CDF at $y_j$ with covariates $\boldsymbol{x}$ can be approximated by the $S$ values $\tilde{F}^{(s)}(y_j|\boldsymbol{x})=G(\tilde{\gamma}_{r(y_j)}^{(s)}-\boldsymbol{x}^{T}\tilde{\boldsymbol{\beta}}^{(s)})$ and the complete conditional CDF can be obtained by a step function connecting $\tilde{F}^{(s)}(y_j|\boldsymbol{x})$ for $j=1,\ldots,J$. The posterior mean distribution conditional on covariates $\boldsymbol{x}$ is approximated by $\tilde{E}^{(s)}[Y|\boldsymbol{x}]=\sum_{j=1}^{J}y_j\tilde{f}^{(s)}(y_j|\boldsymbol{x})$ where $\tilde{f}^{(s)}(y_j|\boldsymbol{x})=\tilde{F}^{(s)}(y_j|\boldsymbol{x})-\tilde{F}^{(s)}(y_{j-1}|\boldsymbol{x})$ and $\tilde{F}^{(s)}(y_0|\boldsymbol{x}) \equiv 0$ so $\tilde{f}^{(s)}(y_1|\boldsymbol{x})=\tilde{F}^{(s)}(y_1|\boldsymbol{x})$. Note that for mixed continuous/discrete outcomes, such as those arising from a detection limit, data below or above the limit do not have a known $y_j$ value. To estimate the $q^{th}$ posterior conditional quantile we first find $y_j^{(s)}=\inf\{y:\tilde{F}^{(s)}(y|\boldsymbol{x})\ge q\}$ and the next smallest value $y_{j-1}^{(s)}$, then use linear interpolation to find quantile $y_q^{(s)}$ where $y_{j-1}^{(s)}<y_q^{(s)}<y_j^{(s)}$. For each of these functionals, point and interval estimates can be obtained by summarizing the $S$ values obtained from the posterior parameter draws without using asymptotic approximations. For example, the mean of the posterior conditional CDF distribution is $\frac{1}{S}\sum_{s=1}^S\tilde{F}^{(s)}(y_j|\boldsymbol{x})$. The 2.5% and 97.5% percentile of the $y_q^{(s)}$ values are the bounds of a 95% credible interval for the $q^{th}$ posterior conditional quantile.
...
To evaluate the <!--long-run frequency--> properties of the Bayesian CPM we generate data from several simulation scenarios:
\begin{align*}
...
For each scenario a second set of simulations was used to evaluate a mixed discrete/continuous outcome such as a lower limit of detection. For scenario (1) and (2) values of $Y<1$ to were set to 1, for scenario (3) values of $Y<0$ were set to 0. The continuous and mixed outcome data based on (1) and (3) were evaluated using a Bayesian CPM with the properly specified probit and loglog links, respectively. For scenario (2) a logit link Bayesian CPM (which implies $\varepsilon \sim Logistic(0,1)$) was used. In each of the six outcome models, three $\boldsymbol{\alpha}$ concentration hyperparameters ($1/J$, $1/(2+(J/3))$, $1/(0.8+0.35J)$) were considered for $p(\boldsymbol{\pi_{.}}|\boldsymbol{\alpha})$ for 18 model and prior combinations. Further, sample sizes $n=25,50,100,200$ and $400$ were considered for each model/prior combination for a total of 90 simulation models. $1,000$ datasets were generated under each simulation model.

We examine the average percent bias of the posterior median for parameters $\beta_1$ and $\beta_2$ and five $\gamma_j$ parameters corresponding to $y$ values spaced across the range of the data. For scenario (1) $\boldsymbol{y}=\{y_1=e^{-1},y_2=e^{-0.33},y_3=e^{0.5},y_4=e^{1.33},y_5=e^2\}$, for scenario (2) $\boldsymbol{y}=\{y_1=e^{-0.5},y_2=e^{0},y_3=e^{0.5},y_4=e^{1},y_5=e^{1.5}\}$ and for scenario (3) $\boldsymbol{y}=\{y_1=-0.3,y_2=0,y_3=0.5,y_4=1.5,y_5=2.5\}$. For the censored outcomes, estimates are only available for the uncensored values of $\boldsymbol{y}$. We also calculate average percent bias of the conditional CDF for $\boldsymbol{y}$ when $X_1=1$ and $X_2=1$, and the conditional median, mean and 20$^{th}$ percentile at $(X_1=1,X_2=1)$ and $(X_1=1,X_2=0)$.
...
We examine the biomarkers Interleukin 6 (IL-6) and Interleukin 1 beta (IL-1-$\beta$); both are right-skewed with 3% and 39% of values censored below the lower limit of detection, respectively. Censored values are set to 0. To account for skewness and censoring we fit Bayesian CPM models using logit, probit, and loglog link functions, noninformative $\beta$ priors and a concentration parameter of either $\boldsymbol{\alpha}=1/J$ or $\boldsymbol{\alpha}=1/(0.8+0.35J)$ for the Dirichlet distribution to estimate the association between BMI and the conditional mean, median, and 90th quantile of each biomarker.

We evaluate convergence of each model using $\hat{R}$ scale reduction factor [@gelman_bayesian_2014] and traceplots of MCMC draws. Model comparison is performed using the difference in expected log predictive density (ELPD) calculated using leave-one-out cross-validation [@vehtari_practical_2017]. Model fit is assessed using graphical checks of the of posterior predictive distribution and posterior predictive p-values [@stern_bayesian_2005; @gelman_bayesian_2014].
...
The top row of figures \@ref(fig:simplt-mn-1) and \@ref(fig:simplt-mn-2) present the bias in the posterior conditional mean for the uncensored simulation outcomes at $(X_1=1,X_2=0)$ and $(X_1=1,X_2=1)$ in scenarios (1) and (2), respectively. For these scenarios, the average percent bias was less than 5\% for all sample sizes and priors.  In contrast, the bottom row of figures \@ref(fig:simplt-mn-1) and \@ref(fig:simplt-mn-2) show the bias in posterior conditional mean estimates for the censored outcomes where a value of $y=1$ was used in the conditional mean calculation for outcomes censored at $Y<1$. Using the censoring threshold value for censored observations can result in inflated average percent bias compared to the uncensored case depending on where the threshold falls in relation to the true conditional distribution and the value chosen for censored observations. For example, the average percent bias of $E(Y|X_1=1,X_2=1)$ for the censored outcome in scenario (1) is around 40\%. 

For scenario (3) the average percent bias for the uncensored outcome ranged from -12.5\% to $<-1.0$\% with larger bias for the $\boldsymbol{\alpha}=1/J$ prior and smaller $n$ (fig. \@ref(fig:simplt-mn-3)). As in the first two scenarios, the censored outcome which used a value of $y=0$ in the conditional mean calculation for observations less than 0 showed a positive shift in average percent bias at $(X_1=1,X_2=1)$ although to a much smaller degree than scenario (1).
...
The simulation results for the conditional posterior median in scenario (1) are shown in figure \@ref(fig:simplt-med-1). Across outcomes, the conditional median estimate had a positive  average percent bias for both $(X_1=1,X_2=0)$ and $(X_1=1,X_2=1)$ with smaller bias for larger sample sizes. There were negligible differences in average percent bias of the median estimates for the three $\boldsymbol{\alpha}$ concentration parameters. The pattern was similar for scenario (2) (supp. fig. \@ref(fig:simplt-med-2)). For both outcomes under scenario (3), average percent bias in the conditional median estimate was smaller at $(X_1=1,X_2=0)$ than $(X_1=1,X_2=1)$. There were only small differences between the three $\boldsymbol{\alpha}$ concentration parameters except for the small sample sizes (fig. \@ref(fig:simplt-med-3)). 

Figure \@ref(fig:simplt-q20-a) presents the results for the posterior conditional 20th percentile in scenario (1). The uncensored outcome estimates are quite biased (between 25\% and 90\%) for the smaller sample sizes. The magnitude of the bias also varies based on the values of the conditioning variables, $X_1$ and $X_2$, with larger bias when the conditional distribution is further from $\beta_1 = \beta_2 = 0$. The estimates of the conditional 20th percentile for the censored outcome in scenario (1) show a similar effect as the censored outcome. When $(X_1=1,X_2=1)$ the true conditional $Q^{0.2}$ falls below the censoring threshold so the estimated $Q^{0.2}$ is the value assigned to the censored category. This value is biased even with large sample size. For scenario (2) the estimates of the posterior conditional 20th percentile with the uncensored outcome are again positively biased for the smaller sample sizes with more bias for the $\boldsymbol{\alpha}=1/J$ concentration prior (figure \@ref(fig:simplt-q20-b)). The estimates of the conditional 20th percentile for the censored outcome show a similar pattern as the uncensored outcome. Under scenario (3) the uncensored outcome estimates of the conditional 20th percentile have reasonably small average percent bias for all the priors and sample sizes except at $(X_1=1,X_2=1)$ when $n=25$. Similar to scanerio (1), the censored outcome estimates show small average percent bias at $(X_1=1,X_2=0)$, but very large average percent bias at $(X_1=1,X_2=1)$ (fig. \@ref(fig:simplt-q20-c)).
...
Simulations were performed using `R` version 3.6.0 (2019-04-26) and `rstan` (Version 2.19.2, GitRev: 2e1f913d3ca3) on a high-performance computing cluster running under CentOS Linux 7 (Core) with 1.90GHz or 2.40GHz Intel Xeon CPUs. MCMC sampling time for the three scenarios is shown in figure \@ref(fig:simplt-samptime). Average per chain sampling time appears to increase exponentially with sample size and was similar across scenarios and priors. <!--The censored outcomes -- with fewer categories and $\gamma$ parameters -- had shorter sampling times.--> 

```{r simplt-samptime, fig.cap="Per chain MCMC sampling time for simulations", fig.align='center', out.width='95%'}
knitr::include_graphics(file.path(wd,"fig","sim_MCMC_sample_times.png"))
```

## Case Study

For each of the two biomarker outcomes six model specifications were fit: probit, logit, or loglog link with $\boldsymbol{\alpha}=1/(0.8+0.35J)$ or $\boldsymbol{\alpha}=1/(0.8+0.35J)$. Each model sampled from 2 chains with 2000 warmup and 4000 total iterations to produce 4000 posterior sample draws for each parameter. For all the biomarker outcome models considered,
traceplots of parameter draws showed no issues with mixing or stationarity; further, all $\hat{R}$ potential scale reduction values were $<1.01$ indicating likely convergence. Table \ref{tab:elpdtab} shows the difference in ELPD for the models for biomarker IL-6 and IL-1-$\beta$, respectively. Based on the difference in ELPD, the CPM with loglog link and $\boldsymbol{\alpha}=1/(0.8+0.35J)$ was used for the both outcomes, however ELPD was similar for the top several models. 
...
The median posterior estimates of the covariate parameters along with 50\% and 90\% credible intervals for the IL-6 model are shown in figure \@ref(fig:il6-par). Age, BMI, and smoking are positively associated with increased IL-6, while CD4 count, male gender, and the Lipoatrophy and Neuropathy cohort are negatively associated with IL-6. Figure \@ref(fig:il6-par-ct) shows the posterior median $\gamma$ estimates along with the 50\% and 95\% credible intervals. Plotting the $\gamma$ estimates against the observed IL-6 values (fig. \@ref(fig:il-6-trans-1)) gives the estimated transformation, $H$.
...
As with the IL-6 biomarker, a plot comparing the observed IL-1-$\beta$ distribution to draws from the posterior predictive distribution (fig. \@ref(fig:il-1-beta-postpred)) does not reveal any serious model misfit. The posterior predictive p-values for variance, skewness, and proportion of observations below the lower limit of detection are shown in table \ref{tab:ppptab2}. There is no indication of serious discrepancy between the model and data for variance and proportion of censored observations. The posterior predicive p-value for skewness is more extreme than the other test quantities indicating a moderate degree of misfit. This seems reasonable given the high level of right-skewness for IL-1-$\beta$.
...
The median posterior estimates of the covariate parameters along with 50\% and 90\% credible intervals for the IL-1-$\beta$ model are shown in \@ref(fig:il-1-beta-par). In contrast to IL-6, there is little association between any of the covariates (except study cohort) and IL-1-$\beta$ level. Figure \@ref(fig:il-1-beta-par-ct) shows the posterior median $\gamma$ estimates along with the 50\% and 95\% credible intervals. Plotting the $\gamma$ estimates against the observed IL-1-$\beta$ values gives the estimated transformation, $H$ (fig. \@ref(fig:il-1-beta-trans)). 
...
Although Bayesian CPM models have been frequently applied to ordinal data when the number of outcome categories is much smaller than the sample size, the extension to continuous or mixed outcomes where the number of categories is close to the sample size can be accomplished with only a few modifications to the prior specification. These modifications provide a versatile model with many advantages including the ability to handle both continuous and discrete ordered outcomes and estimation of the full conditional CDF, along with quantiles and other functionals using a single model fit. Inference for the quantities of interest can be easily interpreted using posterior probability statements and without asymptotic assumptions. The CPM also eliminates the need to specify the transformation required to meet distributional assumptions by estimating this transformation nonparametrically. As a result its parameter estimates are invariant monotonic transformations of the data. 
...
for quantiles further from median and conditional values further from the model where $\boldsymbol{X}=0$ and this bias can be exacerbated by censoring<!--because, by design, we make no parametric assumptions that can provide information here-->.
The model is best suited for cases when the sample size is large enough to inform the full conditional posterior CDF. In our simulations, a sample size of 50 or 100 was required for reasonably unbiased estimates of parameters and other posterior quantities. The choice of Dirichlet prior concentration of magnitude $\approx \frac{1}{J}$ has minimal impact on posterior estimates, except with small sample sizes. <!--Larger concentration parameters will shrink estimates heavily toward intercepts under assumption of equal probability --> <!-- Although the model can be used with mixed continuous/discrete data, care should be exercised when using mixed outcomes or estimating quantities based on the tails of the CDF-->
As with all Bayesian models using MCMC, checks of model convergence, model fit and the posterior distribution are important. This is especially true when modeling a mixed continuous/discrete or when interest lies in quantities conditional on covariates far from the observed mean values.

Finally, there are several of limitations of the current model that present an opportunity for improvement. First, the number of distinct outcome values must be technically be known *a priori*, that is we condition on $J$ categories. In practice, the number of distinct continuous outcome values is unlikely to be known before data collection, so the prior cannot be specified without reference to the observed data. For the same reason, the model cannot accomodate new observations for an unobserved category; once the initial prior is set, there is no way to add categories and all predictions are assumed to come from one of the original categories. It may be possible to overcome this limitation by substituting the Dirichlet prior for a infinite-dimenisonal Bayesian nonparametric analog, such as a Dirichlet process prior at the expense of additional complexity and computation time. Next, the choice of link function, and the implied error distribution on the scale of the latent untransformed data is also assumed to be known. As was seen in the simulations, misspecification of the link function can lead to bias due to the mismatch between the assumed and latent error. This makes interpretation of the $\beta$ parameter estimates challenging because they are only estimated up to a constant of proportionality. If primary interest is not inference for the parameters, specification of the link could be avoided by either estimating the link nonparametrically [@mallick_bayesian_2003; @song_semiparametric_2012; @tang_semiparametric_2018] or using a more flexible mixture link function [@lang_bayesian_1999].
