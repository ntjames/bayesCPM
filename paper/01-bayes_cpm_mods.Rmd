...
We instead adopt a third approach which specifies a prior on $\boldsymbol{\pi_i}$ (for a specified covariate vector) and to utilizes the transformation defined by $G(\cdot)$ to induce a prior on the $\boldsymbol{\gamma}$. [add Betancourt ref]
...
\begin{gather}
\sum_{k=1}^{j}\pi_{.k}=\sum_{k=1}^{j}\left[G^{-1}(\gamma_k)-G^{-1}(\gamma_{k-1})\right]=G^{-1}(\gamma_j)
\end{gather}
so $G\left(\sum_{k=1}^{j}\pi_{.k}\right)=\gamma_j$ defines the inverse transformation. 

Because $\boldsymbol{y}$ has a multinomial distribution a conjugate Dirichlet distribution is a natural choice of prior for $\pi_{.}$. Setting $p(\pi_{.}) \propto \prod_{j=1}^{J}\pi_{.j}^{\alpha_j-1}$ the posterior distribution is
\begin{align}
...
\mathcal{J}_{j+1,j+1}&=\frac{\partial \pi_{.j+1}}{\partial \gamma_j}=\frac{\partial}{\partial \gamma_j}\left[G^{-1}(\gamma_{j+1})-G^{-1}(\gamma_{j})\right]=-g(\gamma_j)\\
\mathcal{J}_{j,j+1}&=\frac{\partial \pi_{.j}}{\partial \gamma_j}=\frac{\partial}{\partial \gamma_j}\left[G^{-1}(\gamma_{j})-G^{-1}(\gamma_{j-1})\right]=g(\gamma_j)
\end{align}
With $\mathcal{J}_{r,c}=0$ for all other entries.
...
\begin{equation}
 ...
 = \begin{vmatrix} 
 1 & g(\gamma_1)  & 0            & 0\\
 1 & -g(\gamma_1) & g(\gamma_2)  & 0\\
 1 & 0            & -g(\gamma_2) & g(\gamma_3)\\
 1 & 0            & 0            & -g(\gamma_3)\\
 \end{vmatrix}
\end{equation}
...
[possible choices 1/J, 1/2 Jeffrey's, 1 uniform, Frank's prior, prior to match MLE? - where $\alpha_j =\frac{1}{J}$ for all $j$]. 
...
Describe results from graphs, compare and contrast the 3 models with focus on 
